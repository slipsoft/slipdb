...
public class ParallelLoaderRunnable implements Runnable {
	...
	public ParallelLoaderRunnable(Table argTable, Parser argParser, Object argLoaderWriteInMemoryLock) {
	...
	// Non thread safe, toujours appelé depuis ParallelLoader
	...
	public void processData() {
		...
		for (int iLine = 0; iLine < linesBufferPosition; iLine++) {
			...
			localReadEntryNb++;
			/*if (localReadEntryNb %  ParallelLoader.updateTotalParsedLinesEvery == 0 && localReadEntryNb != 0) {
		...
		ParallelLoader.totalParsedLines.addAndGet(localReadEntryNb);
